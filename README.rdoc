= Moskrin's changes to acts_as_indexed:

Added a config option called default_partial_word which does partial word matches instead of full-word matches by default.  To force full-word matching when this is enabled, quote the word.  The feature is disabled by default to match the default behaviour of the original.
